...
def _save_solution_and_residuals(sol: PETSc.Vec, res: PETSc.Vec) -> None:
    sol_numpy = sol.getArray(readonly=True)
    res_numpy = np.array(res)  # res.getArray(readonly=True)
    np.savetxt("solution.csv", sol_numpy, delimiter=",")
    np.savetxt("residuals.csv", res_numpy, delimiter=",")
...
def SolveProblem(measure_errors=False):
    ...
    _save_solution_and_residuals(u_sol, res)
...
print(
    "Memory usage in (Gb) = ", resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1e6
)
